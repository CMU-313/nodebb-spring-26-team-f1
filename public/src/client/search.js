...
define('forum/search', [
	...
	function getSearchDataFromDOM() {
		...
		if (['posts', 'titlesposts', 'titles', 'bookmarks'].includes(searchData.in)) {
			...
			// answered filter (checkbox) - send '1' when checked so server receives a truthy value
			searchData.answered = form.find('#filter-answered').length && form.find('#filter-answered').is(':checked') ? '1' : undefined;
		}
	...
	function fillOutForm() {
		...
		if (formData) {
			...
			// restore answered checkbox state when present (support true/'true'/1)
			if (formData.hasOwnProperty('answered') && !!formData.answered) {
				$('#filter-answered').prop('checked', true);
			} else {
				$('#filter-answered').prop('checked', false);
			}
